...
### ^^^ not actually needed, kinda already happens
                ...
                if b[0] == 0:
                    ...
                    for ispb in range(nspbps_v):
                        if fname_phases is None:
                            ...
                            phases = phases*np.ones((nk, num_bands_in,))
                            phases = np.exp(2 * np.pi * 1.0j * phases.T) / np.sqrt(float(nspbps_v))
    ...
    for b in blocks_c:
        ...
        if single_band:
            ...
            for ispb in range(nspbps_c):
                ...
                phases = np.random.random((num_bands_in,))
                phases = phases*np.ones((nk, num_bands_in,))
                phases = np.exp(2 * np.pi * 1.0j * phases.T) / np.sqrt(float(nspbps_c))
