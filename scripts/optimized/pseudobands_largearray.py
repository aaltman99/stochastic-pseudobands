...
# same as pseudobands_opt.py but handles large arrays (oom errors and such). 
# not expected to be used except for when nslice is small
# typically converged calculations won't have to deal with this.
...
def check_and_block(fname_in = None, fname_in_q = None, nv=-1, nc=100, nslice_v = 10, nslice_c = 100, uniform_width=None, max_freq=0., nspbps_v=1, nspbps_c=1, verbosity=0, **kwargs):
    ...
    if nv != -1:
        ...
        try:
            ...
            logger.error(f'More total valence states ({nb_out_v} = copied + SPBs) than original valence bands ({ifmax}) ==> no computational savings in GW steps. Choose smaller nv, nspbps_v, or larger efrac_v')
            raise err
    ...
    if nc != -1:
        ...
        try:
            assert nb_out_c <= nb_orig - ifmax
        except AssertionError as err:
            logger.error(f'More total conduction states ({nb_out_c} = copied + SPBs) than original conduction bands {nb_orig - ifmax} ==> no computational savings in GW steps. Choose smaller nc, nspbps_c, or larger efrac_c')
            raise err
...
def pseudoband(qshift, blocks_v, blocks_c, ifmax, fname_in = None, fname_out = None, fname_in_q = None, fname_out_q = None, fname_in_NNS = None, fname_out_NNS = None, nv=-1, nc=100, nspbps_v=1, nspbps_c=1, single_band=False, copydirectly=True, verbosity=0, **kwargs):
    try:
        ...
        if nc != -1:
            ...
            shape = f_in['wfns/coeffs'][0, :, :].shape
            coeff_out = np.zeros(shape)
            shape = f_in['mf_header/kpoints/el'][:, :, 0].shape
            en_out = np.zeros(shape)
            chunksize = 2000
            ...
            for b in blocks_c:
                ...
                if single_band:
                    ...
                    if num_bands_in <= chunksize:
                        ...
                        # chunking to handle large arrays (avoid oom errors)
                        coeff_out.fill(0)
                        en_out.fill(0) # reset arrays 

                        for ichunk in range(int(np.floor(num_bands_in / chunksize)) + 1):
                            if verbosity > 1:
                                logger.info(f'chunk index: {ichunk}')

                            slc = slice(b[0] + ichunk*chunksize, min(b[0] + (ichunk+1)*chunksize, b[1] + 1))

                            coeff_out += f_in['wfns/coeffs'][slc, :, :].sum(axis=0)
                            en_out += f_in['mf_header/kpoints/el'][:, :, slc].sum(axis=-1)

                        f_out['wfns/coeffs'][ib, :, :] = coeff_out
                        f_out['mf_header/kpoints/el'][:, :, ib] = en_out / num_bands_in # mean over slice

                        if verbosity > 1:
                            logger.info(f'energy of slice {b}: {en_out / num_bands_in}')

                        ib += 1

                else: # nspbps > 1 
    #                 if nspbps >= num_bands_in:
    #                     f_out['wfns/coeffs'][0:nb_out_v+nc, :, :] = f_in['wfns/coeffs'][0:ifmax+nc, :, :]
    #                     f_out['mf_header/kpoints/el'][:,:,0:nb_out_v+nc] = f_in['mf_header/kpoints/el'][:,:,0:ifmax+nc]
                    ...
                    if num_bands_in <= chunksize:
                        ...
                        for ispb in range(nspbps_c):
                            ...
                            ### FIXME: why not just define phases as phases = np.random.random(num_bands_in * nk) and just tensordot without any loop?
                        ...
                        for ispb in range(nspbps_c):
                            ...
                            for ichunk in range(int(np.floor(num_bands_in / chunksize)) + 1):
                                ...
                                chunkstart = b[0] + ichunk*chunksize
                                chunkend = min(b[0] + (ichunk+1)*chunksize, b[1] + 1)
                           
                                slc = slice(chunkstart, chunkend)
                                print(slc)

                                coeff_chunk = f_in['wfns/coeffs'][slc, :, :, :].view(np.complex128)
                                # print(coeff_chunk.nbytes)
                                phases_chunk = np.random.random((chunkend-chunkstart, nk,))
                                phases_chunk = np.exp(2 * np.pi * 1.0j * phases_chunk) 

                                if verbosity > 1:
                                    logger.info(f'phases.shape for SPB {ib}: {phases_chunk.shape}')
                                    logger.info(f'coeffs.shape for SPB {ib}: {coeff_chunk.shape}')
                                    logger.info(f'chunk slice: {(chunkstart, chunkend)}')

                                coeff_out += np.concatenate([np.tensordot(coeff_chunk[:,:,cum_ngk[k]:cum_ngk[k+1]], phases_chunk[:,k], axes=(0, 0)) for k in range(nk)], axis=-2).view(np.float64)
                                en_out += f_in['mf_header/kpoints/el'][:, :, slc].sum(axis=-1)

                            # SPBs are normalized to the DOS / sqrt(number_pseudobands)
                            f_out['wfns/coeffs'][ib, :, :] = coeff_out / np.sqrt(float(nspbps_c))
                            f_out['mf_header/kpoints/el'][:, :, ib] = en_out / num_bands_in
...
if __name__ == "__main__":
    ...
    parser.add_argument('--single_band', help='Use a single band instead of a stochastic combination', type=str_to_bool, nargs='?', const=True, default=False)
